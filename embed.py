...
from simcspr.data_utils import (
    get_dataloaders,
    _image_to_rgb,
    _safe_to_tensor,
)
...
def run(args=None):
    ...
    if args.phis == "dinov2":
        ...
        model, preprocess = clip.load(
            phi_to_name[args.phis], device=device, download_root=ckpt_dir
        )
        model.eval()
        print(
            "Model parameters:",
        ...
        preprocess.transforms[2] = _image_to_rgb
        preprocess.transforms[3] = _safe_to_tensor

    trainloader, valloader = get_dataloaders(
        args.dataset, preprocess, args.batch_size, args.root_dir
    )
    ...
    np.save(f"{representations_dir}/{args.dataset}_repr_train.npy", feats_train)
    np.save(f"{representations_dir}/{args.dataset}_repr_val.npy", feats_val)

    np.save(f"{representations_dir}/{args.dataset}_y_train.npy", y_train)
    np.save(f"{representations_dir}/{args.dataset}_y_val.npy", y_val)
