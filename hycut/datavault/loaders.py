...
def get_datasets(dataset, transform, root_dir="./data"):
    ...
    if dataset == "food101":
        ...
        """
        train_dataset = dsets.ImageFolder(
            root=os.path.join(data_path, "aircraft/trainval"),
            transform=transform,
        )
        val_dataset = dsets.ImageFolder(
    ...
    #     pass
        ...
        for t in np.unique(tmp_targets):
            ...
            subset.extend(
                np.random.choice(
                    np.where(tmp_targets == t)[0], size=30, replace=False
                )
            )
        subset_val = list(
            set([i for i in range(len(tmp_targets))]) - set(subset)
        )
        ...
        """
        train_dataset = MyFER2013(
            root=data_path, split="train", transform=transform
        )
        val_dataset = MyFER2013(
            root=data_path, split="test", transform=transform
        )
        ...
        """
        tmp_dataset = dsets.EuroSAT(
            root=data_path, transform=transform, download=False
        )
        tmp_targets = np.array(tmp_dataset.targets)
        subset_train = []
        subset_val = []
        ...
        """
        train_dataset = dsets.ImageFolder(
            root=os.path.join(data_path, "CLEVR_v1.0/train"),
            transform=transform,
        )
        val_dataset = dsets.ImageFolder(
        ...
        tmp_dataset1 = dsets.ImageFolder(
            root=os.path.join(data_path, "rendered-sst2/train"),
            transform=transform,
        )
        tmp_dataset2 = dsets.ImageFolder(
            root=os.path.join(data_path, "rendered-sst2/valid"),
            transform=transform,
        )
        train_dataset = ConcatDataset((tmp_dataset1, tmp_dataset2))
        val_dataset = dsets.ImageFolder(
            root=os.path.join(data_path, "rendered-sst2/test"),
            transform=transform,
        )
    ...
    for dataset in all_datasets:
        try:
            ds_train, ds_val = get_datasets(
                dataset, get_default_transforms(), "../data"
            )
            print(
                f"Succefully load {dataset} ! training set {len(ds_train)}, test set {len(ds_val)}"
            )
        except:
